...
const helper = require('./messagesHelper');
...
fns.newTextMessage = (message, next) => {
    Joi.validate(message.data, schema.textMessage, (err, messageData) => {
        ...
        let action = helper.testForAction(messageData.message);
        if (action) {
            // TODO
            messageData.message_type = 'giphy';
        }
        Promise.all([action(messageData.message)])
            .then(result => {
                if (result[0] === true) {
                    return;
                }
                console.log(result);
                messageData.message = result[0];
            })
            .then(() => db.insertOne(messageData, 'messages'))
            .then(dbResponse =>
                next(null, {_id: dbResponse.insertedId}))
            .catch(next);
    });
};
...
fns.getMessagesByConversationId = (message, next) => {
    Joi.validate(message.data, schema.messagesByConversationId, (err, messageData) => {
        ...
        db.findMessagesByConversationId(messageData.conversation_id, messageData.query)
            .then(dbResponse => next(null, dbResponse))
            .catch(next);
    });
};
